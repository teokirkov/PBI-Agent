...
| `docs/best-practices/` | Team best-practices reference (populated — the authoritative source for conventions) |
...
- **Visuals are best-effort.** TMDL/PBIP doesn't lend itself to reliably
  ...
  Power BI Desktop.
- **`.xlsx` reading is enabled** via `Bash(python3:*)` /
  `Bash(pip install:*)` in `.github/workflows/claude.yml` — see
  `docs/decisions/0001-widen-bash-scope-for-xlsx.md` for why this scope and
  not plain `Bash(*)`. `python3` with `pandas`/`openpyxl` (installed on
  demand) is the expected way to profile Excel sources.
