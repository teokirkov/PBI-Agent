...
| 2026-09-01 | Human, second Desktop attempt | Past the schema fix, Desktop's TMDL parser then rejected `relationships.tmdl` with `InvalidLineType: Unexpected line type: Empty!` at line 6 — a blank line sitting between two `///` (doc-comment) blocks: an unattached file-header comment (lines 1-5) followed by a blank line, then a second `///` block attached to the first `relationship`. TMDL's parser apparently treats `///` as opening "pending documentation" state that a blank line cannot appear inside before it attaches to a declaration — a genuine syntax rule, not something specific to this content. **Root cause, generalized:** any `///` block not immediately (no blank line) followed by the object it documents is invalid; use `//` for anything not directly attached to a declaration. Fixed by changing the file's leading header from `///` to `//`. Scanned every other `.tmdl` file in the project for the same pattern (blank line immediately after a `///` line) — none found, so this looks isolated to `relationships.tmdl`. **Still not verified**: whether the project opens cleanly past this point. This finding should go into `.claude/skills/pbip-tmdl-structure/SKILL.md` as a TMDL authoring rule, not just this file's fix. |
